<template>
  ...
  <table v-else-if="$scopedSlots.child">
    ...
    <tbody
      ...
      @dblclick="toggleChild(index)" v-bind:key='index'
    >
      <tr>
    ...
    <tfoot
      v-if="itemsPerPage != -1 && this.filteredData.length > this.itemsPerPage"
    >
      {{
  ...
  <table v-else>
    ...
    <tbody>
      <tr v-for="(entry) in paginatedData" v-bind:key='entry'>
        <td v-for="key in columns" v-bind:key='key'>
          <slot :name="key" :entry="entry">
            {{ entry[key] }}
          </slot>
...
export default {
  ...
  data () {
    const sortOrders = {}
    let colAry = this.getColumns(
      this.columnsToDisplay,
      this.data,
      this.columnsToNotDisplay
    )
    if (colAry) {
      colAry.forEach((key) => (sortOrders[key] = 1))
    }
    const childShow = []
    if (this.childInitHide && Array.isArray(this.data)) {
    ...
    getColumns (columns, data, columnsToRemove) {
      ...
      if (columns.length === 0) {
        ...
        if (this.aggregateColumns) {
          ...
          foundColumns = data ? Object.keys(data[0]) : []
        }
